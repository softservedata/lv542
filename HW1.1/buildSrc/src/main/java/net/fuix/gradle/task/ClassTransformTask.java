...
public class ClassTransformTask extends DefaultTask {
	...
	void doAction(IncrementalTaskInputs inputs) throws IOException {
		if(!inputs.isIncremental()) {
			...
			for(File file : dirsToCheck)
				if(file.exists() && FileUtils.isDirEmpty(file.toPath())) file.delete();
		}
	}
	...
	private void processClass(String path, List<ISpecialTransformer> transfs) {
		if(transfs == null || transfs.isEmpty()) return;
		try {
			ClassNode classNode = new ClassNode();
			ClassReader classReader = new ClassReader(org.apache.commons.io.FileUtils.readFileToByteArray(new File(inputDir, path)));
			...
			org.apache.commons.io.FileUtils.writeByteArrayToFile(new File(outputDir, path), writer.toByteArray());
		} catch(IOException e) {
			...
			for(Object o : node.methods) {
				...
				for(ListIterator<AbstractInsnNode> it = m.instructions.iterator(); it.hasNext(); ) {
					...
					if(insnNode.getOpcode() == Opcodes.LDC) {
						...
						String replacement;
						if(ldc.cst instanceof String && (replacement = replaceMap.get(ldc.cst)) != null) ldc.cst = replacement;
					}
				}
			}
		}
					...
					for(int x = 0; x < ann.values.size() - 1; x += 2) {
						...
						if(value instanceof String) {
							...
							for(int j = 0; j < arr.length; j++) {
								...
								if(replacement != null) arr[j] = replacement;
							}
						}
					}
				}
